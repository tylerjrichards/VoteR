...
#'
#' This function allows you to read in your state voter files into R easily
#' @param State Just enter the state you're working with currently
#' @keywords
#' @export
#' @examples
...
Voter_file <- function(State, filelocation, filename) {
  if (State == "Georgia") {
    ...
    Kansas = read.table(file = filename, header = FALSE, fill = TRUE, col.names = readcols, nrows = 10000, sep = "\t")
    Kansas = Kansas[-1, ]
    Kansas$state <- "Kansas"
    return(Kansas)
  }
    ...
    setwd(filelocation)
    Kentucky <- read.table(file = filename, fill = TRUE, nrows = 100, sep = "\t", header = TRUE)
    return(Kentucky)
  }
  else if (State == "NY"){
    setwd(filelocation)
    ...
    New_York <- read.table(file = filename, col.names = nycolumns, fill = T, nrows = 100, sep = ",", header = F)
    return(New_York)
  }
